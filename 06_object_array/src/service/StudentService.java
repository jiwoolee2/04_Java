...
/* Service : 프로그램의 실질적인 기능(비즈니스 로직)을 제공하는  클래스
 * 
 * 
 */
public class StudentService {
	...
	// - 인덱스 : 0~4
	// - 각 인덱스 요소의 초기값 : null (아직 주소 저장 x , 참조 객체 x)
	// 기본 자료형(int, char..) <-> 참조 자료형
	...
	public StudentService() {
		...
		for(StudentDTO std : students) {
			...
			// random.nextInt(101) : 0이상 101미만이 정수형 난수 생성
	...
	public boolean addStudent(StudentDTO student) {
		...
		// students에 null이 존재하지 않을 경우 == 비어있는게 없음
		return false;
	}
	...
	public StudentDTO selectIndex(int index) {
		// index가 0~4가 아니면 오류발생
		// index가 배열 범위를 벗어난 경우 null 반환
	...
	public StudentDTO selectName(String targetName) {
		...
		for(StudentDTO std : students) {
			...
			// 비교 연산자(A==B)
			// -A와 B에 저장된 값이 같으면 true, 다르면 fasle 반환
			// 변수에 저장된 값만 비교
			
			// boolean A.equals(B)
			// - A가 참조하는 객체의 필드와
			//   B가 참조하는 객체의 필드가 같은지 비교하여
			//   같으면 true, 다르면 fasle 반환
			// 객체가 가지고 있는 실제 값을 비교
			...
			if(std.getName().equals(targetName)) {
				...
				// 위에 값 둘다 string인데 이거는 참조자료형임. 기본자료형과 다르게 주소를 저장. 값이 아니라
				// std.getName()과 targetName에 저장된 값이 같을 경우
				// - 참조형은 주소를 저장하고 있다!
				// - 생성된 객체는 기본적으로 서로 다른 곳에 생성된다
				// -> 저장된 이름과 입력된 이름이 같아도 같지 않다고 발생할 수 있다
			}
		}
	// if문이 하나도 실행되지 않은 경우
	return null;
