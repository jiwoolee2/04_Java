...
public class ListService {
	...
	public void test2() {
		...
		for(int i=0 ; i<menuList.size(); i++) {
			String menu = menuList.get(i); // i번쨰 인덱스 값 얻어오기
			// -> 타입 검증 X, 다운 캐스팅 X
	 ...
	 * 
	 * -> 프로그래밍 과정에서 미처 생각하지 못한 비효율적인
	 * 요소를 쉽게 개선하기 위해서
	 * 
	 *  1. ArrayList
	 *  - 배열 처럼 요소가 붙어있음
	 *  - 장점 : 검색 속도가 빠름
	 *  - 단점 : 중간에 추가, 수정, 삭제가 비효율적임
	 *  
	 *  2. LinkedList
	 *  - 요소를 서로 참조하는 형식으로 되어있음
	 *  - 장점 : 중간에 추가, 수정, 삭제가 효율적임 
	 *  - 단점 : 검색 속도가 느림
	 */
...
//		}
	...
	public void test4() {
		...
		// -> Collection 상속 객체(List, Set)을 현재 리스트의 제일 뒤에 추가
		// - E : 현재 list에 제한된 타입(아래에서는 Parent)
		// (?는 E이거나 E의 자식 객체여야한다는 뜻)
		// - ? : Parent or Child가 들어갈 수 있다
		
		list3.addAll(list1); // List<Parent>
		list3.addAll(list2); // List<Child>
		
//		List<String> temp = new ArrayList<String>();
//		list3.addAll(temp); // String은 Parent의 상속 관계가 아니어서 오류발생
		
		
		for(Parent p : list3) {
			System.out.println(p); // p.toString 호출됨
		}
