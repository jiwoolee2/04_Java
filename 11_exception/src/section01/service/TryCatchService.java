...
public class TryCatchService {
	 ...
	 * 
	 * catch(예외 참조 변수){ }
	 * - try에서 던져진 예외를 ()내 예외 참조 변수가 잡아서 처리하는 구문
	 * -> 잡아서 처리하면 예외가 사라짐
	...
	public void test2() {
		...
		try {
			...
			System.out.println("0으로 나눌 수 없습니다");
				...
				// catch는 여러번 작성 가능!!!!
				// -> try에서 예외 발생 시 던져진 예외를 잡아서 처리할 수 있는
				//    catch문을 ㅜ이에서 부터 순서대로 탐색
	...
	public void test3() {
		...
		try {
			...
			System.out.println("몫 : " + num1/num2);
				...
				// ArithmeticException와 InputMismatchException의
				// 공통 부모 타입을 매개 변수로 선언
	...
	public void test5() {
		...
		do {
			...
			try {
				...
				input = sc.nextInt();  // 여기서 InputMismatchException 발생 가능성 있음
	 ...
	 * 
	 * - 중간에 ㅍ로그램이 종료되거나 메서드 리턴되어도 무조건 실행
	 * 
